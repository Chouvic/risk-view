...
import { Claim, PairTable, Path, Takeaway } from "../parts/ui";

/** The brief, mapped onto artifacts. */
export function Brief() {
  return (
    <ReportSection id="brief">
      <Claim>Three computations, three written parts, one implementation.</Claim>
      ...
      <PairTable
        ...
        rows={[
          ...
          [
            ...
            <>
              <Path>analytics/hedge.py</Path> — section 05, with the one interpretation call below.
            </>,
          ],
      ...
      <Takeaway>
        One interpretation call. The brief says hedge “100% of NAV”, but on the final cashflow date
        NAV equals the terminal value while nothing is outstanding — selling that forward would be
        selling currency already received. So each NAV point carries two numbers:{" "}
        <strong className="font-medium">nav</strong> for reporting,{" "}
        <strong className="font-medium">open_exposure</strong> for sizing trades.
      </Takeaway>
    </ReportSection>
  );
}
